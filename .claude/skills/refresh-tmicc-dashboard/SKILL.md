...
This file holds an array under `documents` — one entry **per fiscal year** (not per filing), selectable via a dropdown in the console (`index.html`'s `#ar-doc-select`), most recent first. Currently four: FY2025 (from the real Annual Report, Form 20-F) down to FY2022 (from combined carve-out financials inside the pre-listing registration statement, Form 20FR12B — TMICC didn't exist as a standalone filer that early). The FY2025 entry only changes when TMICC files a new Annual Report (next one expected ~Feb 2027) or when a previously-unread filing (the two 2026 Form 6-Ks are a standing gap) gets read in — re-check TMICC's investor relations site + SEC EDGAR each pass to confirm you're still working from the current filing. When FY2026 actually gets an Annual Report, add it as a **new entry** with its own `id`/`label` (e.g. `fy2026`) rather than overwriting FY2025 — the dropdown picks up new entries automatically, no `index.html` changes needed. Every checkpoint needs a real page/item citation — don't add one without re-opening the source document. FY2022-2024 entries have an empty `report_map` (they're figures inside a shared document, not standalone filings) — the frontend hides that section automatically when the array is empty, don't force a fake map to fill it. Keep honest about what each year's `source_document.filing_type` says it actually is — never relabel a registration statement's carve-out figures as if they were a real annual report's. Each year also carries a `strategic_analysis` array — real interpretation (insight/why_it_matters/what_to_watch), not fact, kept separate from `strategic_checkpoints` for the same reason the Consultant's Brief is its own file. When updating a year, ask whether any new fact closes a `what_to_watch` item, confirms or contradicts an `insight`, or opens a new one — don't just append checkpoints without revisiting whether the analysis built on top of them still holds.
