...
This file holds an array under `documents` — one entry per filing, selectable via a dropdown in the console (`index.html`'s `#ar-doc-select`). Currently two: the FY2025 Annual Report (Form 20-F) and the pre-listing registration statement (Form 20FR12B). The Annual Report entry only changes when TMICC files a new one (next one expected ~Feb 2027) or when a previously-unread filing (the two 2026 Form 6-Ks are a standing gap) gets read in — re-check TMICC's investor relations site + SEC EDGAR each pass to confirm you're still working from the current filing before spending time on it. If a genuinely new TMICC-filed document surfaces (another Form 6-K, a future Annual Report), add it as a **new entry** in the `documents` array with its own `id`/`label` rather than overwriting an existing one — the dropdown picks these up automatically, no `index.html` changes needed. Every checkpoint needs a real page/item citation — don't add one without re-opening the source document. Keep honest about what each document actually is (an annual report is not the same thing as a registration statement) — never relabel one as the other just to make the "previous reports" list look longer.
